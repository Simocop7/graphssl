...
\subsection{Preliminary Empirical Results: Citation Networks}
\label{sec:results-citation}
As a first execution of the evaluation protocol, we trained BGRL with a frozen GIN-2L encoder ($d{=}256$) on the three standard citation networks --- Cora, CiteSeer, and PubMed --- using their public Planetoid train/val/test splits. Training is full-batch for 300 steps with symmetric augmentation (edge dropout $p{=}0.5$, feature masking $p{=}0.2$) and the AdamW optimizer ($\text{lr}{=}5{\times}10^{-4}$, weight decay $10^{-5}$). \Cref{tab:accuracy} reports test-set accuracy under both evaluation heads --- a linear probe (Adam) and a $k$NN classifier ($k{=}5$) --- averaged over 10 seeds, with the sample standard deviation.

\begin{table}[htbp]
\centering
\caption{Test Accuracy (\%) on Citation Networks (BGRL, GIN-2L, $d{=}256$, 10 seeds, public split)}
\label{tab:accuracy}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Dataset} & \textbf{Linear Probe} & \textbf{$k$NN ($k{=}5$)} \\ \midrule
Cora     & 62.39 $\pm$ 3.71 & 58.61 $\pm$ 3.76 \\
CiteSeer & 50.08 $\pm$ 1.58 & 43.07 $\pm$ 3.61 \\
PubMed   & 69.18 $\pm$ 2.32 & 66.59 $\pm$ 2.88 \\ \bottomrule
\end{tabular}
\end{table}

These results confirm that the complete pipeline --- augmentation, EMA teacher-student optimization, and both evaluation heads --- executes correctly end-to-end and yields representations substantially above the random-guess baseline (14.3\%, 16.7\%, and 33.3\% for Cora, CiteSeer, and PubMed, respectively, given their class counts).

\Cref{tab:bgrl-comparison} places these numbers alongside the original BGRL paper's own small-dataset evaluation (Appendix C, Table 7 of \cite{bgrl}). The comparison is not strictly apples-to-apples, and we report the methodological differences explicitly rather than let the numbers speak alone: \cite{bgrl} averages over 20 random dataset splits rather than the fixed public Planetoid split used here (the original authors themselves note the fixed split is known to be unreliable for evaluating GNNs \cite{shchur2018pitfalls}); their encoder follows the GRACE setup \cite{zhu2020graph}, most likely a 2-layer GCN rather than our GIN; and their linear head is an $\ell_2$-regularized scikit-learn logistic-regression classifier with a hyperparameter search over the regularization strength, rather than our untuned Adam-trained linear layer.

\begin{table}[htbp]
\centering
\caption{Test Accuracy (\%): our BGRL run (public split, GIN-2L, untuned linear probe) vs.\ the original BGRL paper's small-dataset evaluation (20 random splits, GRACE-style encoder, tuned scikit-learn logistic regression)}
...
This work & BGRL (linear) & 62.39 $\pm$ 3.71 & 50.08 $\pm$ 1.58 & 69.18 $\pm$ 2.32 \\ \bottomrule
\end{tabular}
\end{table}

The gap is consistent with the configuration differences above rather than a defect in the implementation: our run intentionally omits per-dataset hyperparameter search, uses symmetric rather than asymmetric augmentation strength across the two views, and evaluates through an untuned linear layer rather than a regularization-swept logistic regression classifier. Closing this gap under a matched protocol is left as future work; the present results are offered as a correctness and reproducibility check, not a state-of-the-art claim. Medium- and large-scale OGB datasets \cite{ogb} (ogbn-arxiv, ogbg-molhiv, ogbg-molpcba) require dedicated compute beyond a laptop-class CPU and are deferred to a forthcoming evaluation pass on NECSTLab infrastructure, with the explicit caveat that memory-intensive global clustering methods (e.g., AFGRL) will require the memory optimizations discussed in \Cref{sec:limitations} to run on the larger entries.
